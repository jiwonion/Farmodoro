...
- 성장 중인 밭 하단에는 물주기·코인 성장 버튼을 항상 표시해 선택 없이 바로 사용할 수 있다. 성장 상태는 숫자로 표시하며 노란 성장 칸은 사용하지 않는다.
- 밭을 누르면 작물 이름·성장 단계·시듦까지 남은 시간이 밭 위의 작은 창에 나타난다. 수확·폐기도 상세 창에서 조작한다.
- 상단 기능 버튼은 없다. 수확물 상자·씨앗 상자·용품 거치대를 누르면 해당 보관함, 우편함은 우편소, 화로는 주방, 게시판은 랭킹, 판매대는 중앙 팝업 상점을 연다. 상점은 배경·닫기 버튼·Esc로 닫으며, 씨앗 판매대에는 해당 씨앗과 수확 작물의 보유 수량을 표시한다.
...
- 집중 탭의 농장 배경 버튼은 현재 농장 전경을 타이머 뒤에 표시한다. 장착 테마·밭·작물·시설을 반영하며 농장 갱신 시 함께 갱신한다. 조작 요소는 제거하고 전체 화면에서도 지도 비율을 유지하면서 화면을 가득 채우도록 확대한다. 상단 풍경을 기준으로 맞추고, 가로는 중앙 정렬하며 넘치는 가장자리는 잘린다. 다시 누르면 기존 배경으로 돌아가며, 이미지 배경 적용·기본 배경 선택 시 농장 배경을 해제한다. 선택은 현재 세션에 적용된다.
...
기본 초원과 봄 초원에는 초록 풀잎 12개가 왼쪽 위에서 오른쪽 아래로 대각선으로 날리는 바람 효과를 표시한다. 벚꽃 풍경에는 꽃잎, 겨울 풍경에는 눈을 표시한다. 효과는 클릭을 막지 않으며 동작 줄이기 설정에서 숨긴다. 상점 미리보기도 같은 맵을 사용하며 재화를 소비하거나 장착 상태를 바꾸지 않는다.
