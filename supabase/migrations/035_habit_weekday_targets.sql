-- Optional per-weekday targets. Missing keys fall back to target_value.
alter table public.habits
  add column if not exists target_by_weekday jsonb not null default '{}'::jsonb;

alter table public.habits
  drop constraint if exists habits_target_by_weekday_object;

alter table public.habits
  add constraint habits_target_by_weekday_object
  check (jsonb_typeof(target_by_weekday) = 'object');
