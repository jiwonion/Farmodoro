-- Split the crop/food buy stands visually and expand their daily rotations.
-- Existing rotations are rerolled on the next farm-state read because their
-- current cardinality no longer matches the expected 6 / 7 offer counts.
...
alter table public.farm_market_rotations
  add constraint farm_market_food_offer_count
    check (cardinality(food_offer_ids) <= 6);

alter table public.farm_market_rotations
  drop constraint if exists farm_market_crop_sell_offer_count;
alter table public.farm_market_rotations
  add constraint farm_market_crop_sell_offer_count
    check (cardinality(crop_sell_offer_ids) <= 7);
