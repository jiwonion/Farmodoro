-- Seven unique offers per seed, food and crop stand.
-- Existing six-item rotations refill on the next farm read; refresh tickets
-- use the same count. Cosmetics remain at three offers.
begin;

alter table public.farm_market_rotations
  drop constraint if exists farm_market_seed_offer_count,
  drop constraint if exists farm_market_food_offer_count,
  drop constraint if exists farm_market_crop_sell_offer_count;
alter table public.farm_market_rotations
  add constraint farm_market_seed_offer_count check (cardinality(seed_offer_ids) <= 7),
  add constraint farm_market_food_offer_count check (cardinality(food_offer_ids) <= 7),
  add constraint farm_market_crop_sell_offer_count check (cardinality(crop_sell_offer_ids) <= 7);
...
begin
  ...
  if food_count <> 7 then
    select array_agg(recipe_id) into rolled_foods
    from (select recipe_id from public.farm_recipe_catalog order by random() limit 7) as t;
    update public.farm_market_rotations set food_offer_ids = rolled_foods
    where user_id = p_user_id and rotation_date = today;
  end if;

  if crop_sell_count <> 7 then
...
begin
  ...
  if p_item_id = 'seedMarketRefresh' then
    ...
    from (
      select crop_id || ':' || (case when random() < 0.5 then 5 else 10 end) as offer
      from public.farm_crop_catalog order by random() limit 7
    ) as t;
